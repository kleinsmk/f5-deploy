...
{
    #############if you change this code to usse a psocustom object cast it will be faster"
    $ReturnArray = "" | Select-Object -Property Condition, Ip, Subnet
 
    if($un2 -eq ($ma2 -band $un1)){
        $ReturnArray.Condition = $True
        $ReturnArray.Ip = $addr1
        $ReturnArray.Subnet =$addr2
        return $ReturnArray
    }else{
